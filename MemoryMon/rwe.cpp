...
static void RweSwitchToNormalMode(_Inout_ ProcessorData* processor_data);
...
static ActiveMemoryPolice g_active_mem_police;
...
void RweAddPolice(const MEMORY_POLICY & one_policy) {
	return g_active_mem_police.add(one_policy);
}

/*  */
void RwePrintPolicies() {
	return g_active_mem_police.print_policies();
}

/*  */
bool RweShouldWeProtect(const _In_ void* src_address, const _In_ void* dst_address) {
	return g_active_mem_police.is_in_range(src_address, dst_address);
}
...
/*  */
_Use_decl_annotations_ bool RweIsInsideAllocRange(void* address) {
...
_Use_decl_annotations_ static void RweSwitchToNormalMode(
    ProcessorData* processor_data) {
...
_Use_decl_annotations_ static void RwepHandleExecuteViolation(
  ...
  if (RweIsInsideSrcRange(fault_va)) {
    ...
    //  Dst   o   o
    //  Oth   o   o
    RweSwitchToNormalMode(processor_data);
...
//  NT_ASSERT(processor_data->ept_data == processor_data->ept_data_monitor);
  ...
  const auto is_inside_range = RweIsInsideAllocRange(fault_va);

  //ADDBUG const auto is_inside_range = RweIsInsideDstRange(fault_va);
...
//                                PAGE_ALIGN(fault_va));
  ...
  else if (!is_write && fault_va == PsInitialSystemProcess) {
	// HYPERPLATFORM_COMMON_DBG_BREAK();
	const auto pfn = UtilPfnFromVa(g_rwe_zero_page);
	ept_entry->fields.physial_address = pfn;
	UtilInveptGlobal();
  } 
  else if (RweShouldWeProtect(guest_ip, fault_va)){
	  DbgPrint(" ** RwepHandleReadWriteViolation %I64X %I64X **\r\n",
		  guest_ip, fault_va);
	  const auto pfn = UtilPfnFromVa(g_rwe_zero_page);
	  ept_entry->fields.physial_address = pfn;
	  UtilInveptGlobal();
...
//       PAGE_ALIGN(processor_data->rwe_data->last_data.fault_va));
  ...
  if (processor_data->rwe_data->last_data.is_write) {
    if (!MemTraceIsEnabled()) {
	  ...
	  else if (RweShouldWeProtect(
		  processor_data->rwe_data->last_data.guest_ip,
		  processor_data->rwe_data->last_data.fault_va)) {

		  DbgPrint(" ** RweHandleMonitorTrapFlag %I64X %I64X **\r\n",
			  processor_data->rwe_data->last_data.guest_ip,
			  processor_data->rwe_data->last_data.fault_va);
    ...
    }else if (processor_data->rwe_data->last_data.fault_va ==
		PsInitialSystemProcess) {
		// HYPERPLATFORM_COMMON_DBG_BREAK();
		const auto pfn = UtilPfnFromVa(PsInitialSystemProcess);
		processor_data->rwe_data->last_data.ept_entry->fields.physial_address =
			pfn;
		UtilInveptGlobal();
		HYPERPLATFORM_LOG_INFO_SAFE(
			"PsInitialSystemProcess is being read. Returning fake contents.");
	}
	else if (RweShouldWeProtect(
		processor_data->rwe_data->last_data.guest_ip, 
		processor_data->rwe_data->last_data.fault_va)) {
